document.addEventListener("DOMContentLoaded", function () {
  ...
  // ─────────────────────────────────────────────
  // 📦  ROW CACHE
  // Rebuilt every time the infrastructure fires
  // 'tableReady' — that's the moment ALL rows are
  // in #lista before pagination touches them.
  // ─────────────────────────────────────────────
  let rowCache  = [];      // [{ el: tr, text: string }]
  let ownReload = false;   // true when WE fired tableReady — skip re-cache
  ...
  function filtrarTabla() {
    ...
    // ── CLEARED: restore full dataset, re-trigger pagination ──────────
    if (filtro === "") {
      tbody.innerHTML = "";
      rowCache.forEach(r => {
        r.el.style.display = "";
    ...
    // ── ACTIVE: ensure every cached row is in the DOM, then filter ────
    rowCache.forEach(r => {
